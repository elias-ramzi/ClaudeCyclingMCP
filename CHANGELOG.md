...
- **`mywhoosh-activity-import`** — a third bundled skill, and the first that goes the other way:
...
- **A skill can declare `prompt_input: activity` in its frontmatter.** Invoked as an MCP prompt
  with no argument, every skill was introduced with "ask what session to build … confirm the FTP
  before rendering" — an instruction to build a workout, in front of a procedure about a ride
  already sitting on disk. The two upload skills are unaffected; an unrecognised value falls back
  to the old wording rather than dropping the skill.
