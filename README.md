...
- 📝 **One hand-readable spec** — flat JSON with `steady`, `ramp`, `free` and `repeat` blocks. Power is explicit: `power_w` or `power_pct`, never guessed from magnitude ([spec format](docs/spec-format.md)).
...
- 🚴 **Two renderers, one source** — `.zwo` for MyWhoosh and a Garmin `upload_workout` payload, each pinned to a platform quirk that was expensive to learn ([format notes](docs/tools.md#format-notes)).
...
them on its own: [Installing](docs/install.md).
...
How a skill reaches your client, and what each needs to actually run: [Skills](docs/skills.md).

## Documentation

- [Spec format](docs/spec-format.md) — every field, the block types, and how FTP is consumed by each platform.
- [Tools](docs/tools.md) — the full tool reference, plus the `.zwo` and Garmin format rules the renderers are pinned to.
- [Garmin schema provenance](docs/garmin-schema.md) — what the payload shape was derived against, the two silent-failure findings, and verification status.
- [Skills](docs/skills.md) — what each bundled skill does, and the two ways one runs.
- [Testing](docs/testing.md) — the offline gate and the live Garmin round-trip.
...
- [docs/versioning.md](docs/versioning.md) · [CHANGELOG.md](CHANGELOG.md) · [SECURITY.md](SECURITY.md)
...
by default — see [Testing](docs/testing.md).
